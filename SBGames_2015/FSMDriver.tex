...
\subsection{Three-State FSM} \label{subsec:FSM3}
	...
	\toDo{Mention inside track calculating a target speed based on how far the car is from the farthest edge of the track. This distance means the greater path in wich the car can head without any steer changes.}
	...
	\toDo{Besides the states, there is also a learning module that is called whenever the Out of Track state is called. This module records both the speed and the position of the car when it leaves the track. Holding this information the controller is able to slow down in subsequent laps when approaching the critical point. (how is this done?) (specify how close to the critical point the brake process begin)}
...
\subsection{Search for Parameter Values - Genetic Algorithm} \label{subsec:GA}
	...
	scoring system, what is called Elitism\toDo{referencia para elitismo}. Each pair of parents is submitted to
	crossover in order to generate two offspring solutions, and in the end of the process each offspring may
	present mutation - everything according to predefined rates. \toDo{For this work, the crossover rate is 95\%,
	while the mutation rate is 1\%, which are values proposed by Núnez-Letamendia, reference}.
	...
	the references~\cite{GitHub}. \toDo{Devemos fazer isso, certo? Reprodutibilidade.}
