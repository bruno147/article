...
\subsection{Five-state FSM} \label{subsec:FSM5}
	...
	target speed is achieved;\toDo{approaching curve does not implements the farthest sensor policy, this is done in the curve state. Approaching curve in fact moves the car to the opposite side of the curve proportionally to turn's curvature, if there is a critical turn to left the car will position itself to the right, in order to gain speed in the turn. A target position (trackpos sensor) is defined and steer is set to achieve this goal} or, when in situations of \emph{Curve}, the pilot would stop braking while maintaining
...
\subsection{Three-State FSM} \label{subsec:FSM3}
	...
	\toDo{provide picture showing the fartherst sensor in a turn}		
	...
	system to brake when the car begins skidding above a threshold was implemented.\toDo{the controller also tries to return to the track in a "friendly" angle. There were defined a max and a min return angle so the car does not found itself in tricky situations (a picture would help, for real)}
