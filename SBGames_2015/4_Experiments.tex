...
\gnramos{It is important to mention that before each result displayed in that section a warm-up stage was held for 5 laps in each track. -> por conta do modulo de aprendizado, e é assim que o scr  funciona e autopia tambem aprende. o site da competição de 2015 diz que warmup é 5 voltas http://cs.adelaide.edu.au/~optlog/SCR2015/}
...
The Genetic Algorithm used standard configurations from literature \gnramos{inserir *boas* referencias que indicam os valores, prencher tabela - boas referencias são livros e artigos cientificos de periodicos - nao de conferencias}, population size of 30, generations ?, mutation probability ?, crossover ?  \gnramos{justificar a quantidade de geracoes e, assim, o tamanho da populacao (ou vice versa). explicar a selecao e o crossover} % 616 generations, calculated using ~\cite{218485} formula, of 30 individuals
The fitness function in this case is the sum of distances covered in all tracks, considering 10.000 game ticks per track.

\gnramos{aqui GA. elitismo: 4 melhores são separados e mantidos a cada geração.}
\gnramos{10 melhores (de 30) reproduzem. 26 vezes, 1 par é selecionado aleatoriamente,}
\gnramos{e geram 2 individuos, repete 13 vezes -> 26, + 4 melhores = 30. crossover é }
\gnramos{em um ponto aleatorio e há swap.}
\gnramos{chance de crossover: 95\%}
\gnramos{chance de mutação: 1\% -> mutação é, a cada bit, testa a probabilidade e troca.}

\gnramos{arbitrariamente 30 individuos o que, conforme ~\cite{218485} leva a 616 gerações -> por que?}
...
\gnramos{aleatoriamente um hist4, porque tem resultado contra AUTOPIA. não dá para botar bot correndo por ticks, dai não tem na tabela 1.Autopia nao compara contra bots com mesmo carro}
